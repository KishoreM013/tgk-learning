...
export default function DoublyLinkedListPlayground({ structure, onOperationChange }: any) {
  const [values, setValues] = useState(['Head', 'Node 1', 'Tail']);
  const [input, setInput] = useState('New Node');
  const [operation, setOperation] = useState('Insert');
  const [message, setMessage] = useState('Nodes are connected both ways.');
  ...
  const act = () => {
    ...
    if (operation === 'Traverse') setMessage(`Traversed list of size ${values.length} forwards and backwards.`);
  };
